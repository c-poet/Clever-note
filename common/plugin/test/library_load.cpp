...
int main(int argc, char *argv[]) {
        ...
        if (!list.isEmpty()) {
            ...
            for (const auto &item: list) {
                ...
                if (lib->load()) {
                    ...
                    if (initFunc) {
                        debug() << "调用init方法返回" << ((Handle) initFunc)(10, nullptr);
                    }
                    QFunctionPointer startFunc = lib->resolve(PLUGIN_START_FUNC);
                    if (startFunc) {
                        debug() << "调用start方法返回" << ((Handle) startFunc)(1000, nullptr);
                    }
                }
            }
        }
    ...
    if (!libraries->isEmpty()) {
        ...
        for (int i = 0; i < len; ++i) {
            ...
            if (lib->isLoaded()) {
                debug() << "关闭插件：" << lib->fileName() << ":" << lib->unload();
            }
            delete lib;
        }
    }
